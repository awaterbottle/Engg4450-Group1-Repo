...
/*Engg4450 Desmond Edited
 * Added _shiftPressed = false
 *
 */
    ...
    switch (evt)
    {
        case Keyboard::KEY_PRESS:
        {
            switch (key)
            {
                ...
                /**Engg4450 Desmond Edited
                 * Key press event check for shift key
                 */
        ...
        case Keyboard::KEY_CHAR:
        {
            switch (key)
            {
                ...
                default:
                {
                    ...
                    {
                    	/**Engg4450 Desmond Edited
                    	 * Change Char to Upper case if shift pressed
                    	 * and the character is not lower case
                    	 */
                    	if ( _shiftPressed &&
                    		 ( key >= Keyboard::KEY_A && key <= Keyboard::KEY_Z ) ) //Test for lower case, uses ASCII decimal i.e between the range of 97 and 122 inclusive
                    	{
                    		key -= CHAR_TO_CAPS; //Subtracting the char by 32 decimal will give it's CAPITAL equivalent
                    	}
